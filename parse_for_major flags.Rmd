...
The `do_location` parameter flags questionable locations and is set to TURE by default. Set `do_location` to FALSE to exclude location checks from the analysis. 
...
`refield_report` a data frame that provides information about interviews that failed one or more major tests.
The output columns are:
 
 - `_UUID`: Unique identifier for each interview
 - `Internal_ID`: Internal identifier for the interview
 - `Date`: Date of the interivew
 - `Interviewer_ID`: ID of the interview
 - `Interviewer_Name`: Name of the interviewer
 - `Reasons`: Details about the reasons for failure, such as being too short, having high missingness, etc.
